...
# 关键文件路径强制基于脚本目录
data_excel_path = ""  # 初始化为空字符串，稍后由用户选择
...
def get_file_gui(title, filetypes):
    """通过图形界面选择文件"""
    root = Tk()
    root.withdraw()  # 隐藏主窗口
...
def load_global_vars(data_excel_path):
    """加载全局变量并转换为字典"""
    try:
...
def process_sheet(sheet_name, template_path, output_folder, global_vars):
    ...
    for row_index, row in df.iterrows():
        ...
        combinations = list(product(*row_values.values()))
        combo_count = len(combinations)
        
        # 精简文件名：只保留行号和组合序号
        for combo_index, combo in enumerate(combinations, 1):
            content = template
            # 先替换全局变量
            for var_name, var_value in global_vars.items():
            ...
            # 精简文件名：行号 + 组合序号
            filename = f"行{row_index+1}_{combo_index}.json"
            ...
            if combo_index % 100 == 0:
                print(f"  正在处理行 {row_index+1}: {combo_index}/{combo_count}")
...
print("=== 请选择配置文件（config.xlsx） ===")
config_excel_path = get_file_gui(
    "选择配置文件",
    [("Excel文件", "*.xlsx"), ("所有文件", "*.*")]
)

if not config_excel_path:  # 用户取消选择
    graceful_exit("❌ 错误：未选择配置文件")
    
if not os.path.isfile(config_excel_path):
    graceful_exit(f"❌ 错误：文件 {config_excel_path} 不存在")

print("=== 请选择数据文件（data.xlsx） ===")
data_excel_path = get_file_gui(
    "选择数据文件",
    [("Excel文件", "*.xlsx"), ("所有文件", "*.*")]
)
...
global_vars = load_global_vars(data_excel_path)  # 从数据文件加载全局变量
